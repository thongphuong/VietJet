...
namespace TrainingCenter.Scheduler
{
                    ...
                    if (item.IdMethod is (int)UtilConstants.ScheduleMethod.Mail)
                    {
                        ...
                        if (schedule.CAT_MAIL.Code == UtilConstants.TypeSentEmail.SendMailReminderCourse.ToString())
                        {
                            ...
                            foreach (var item_member in courde_detail)
                            {
                                ...
                                {
                                    if (item_trainee?.Course_Result.Any(a => a.Re_Check_Result == null ? (a.First_Check_Result != "F" && a.First_Check_Result != null) : a.Re_Check_Result != "F") == true)
                                    {
                                        continue;
                                    }
                                    else
    ...
    public class Schedules : IJob
    {
        ...
        private static void TypeRepeat()
        {
            using (var dbContext = new EFDbContext())
            {
                ...
                var entity = repoSchedulesMethod.GetAll(a => a.Schedule.IsDelete == false && a.Schedule.IsActive == true && a.IdType == (int)UtilConstants.ScheduleType.Repeat);
                if (!entity.Any())
                {
                    uow.Dispose();
                    return;
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                using (var dbContext = new EFDbContext())
                {
                    ...
                    ////////////////////Sent mail in TMS_SentEmail/////////////////////////////////////
                    var entitySentMail = repoTmsSentEmail.GetAll(a => a.flag_sent == 0 && a.Is_Deleted == false && a.Is_Active == true).Take(20).ToList();
                    if (entitySentMail.Any())
                    {
                        ...
                        foreach (var item in entitySentMail)
                        {
                            ...
                            {
                                if (item.AttachFileType ==
                                    (int)UtilConstants.TypeSentEmailAttachFileType.ReportTrainingPlan)
                                {
                                    var file = ScheduleMethod.ExportExcelCourse();
                                    if (MailUtil.SendMail(mailTo, item.subjectname ?? item.CAT_MAIL.SubjectMail, item.content_body, file))
                                    {
                                        item.flag_sent = 1;
                                    }
                                    else
